...
def plot_graph(function, points):
    plt.title("Modifield Euler Method Equation Solver")
    ...
    plt.plot(x, y, color='red', zorder=2, label='Lagrange polynomial', linewidth=2)
    ...
    x_points = []
    y_points = []
    plt.scatter(points[0][0], points[0][1], color='purple', zorder=5, label='the starting points')
    for i in range(1, len(points)):
        x_points.append(points[i][0])
        y_points.append(points[i][1])
    plt.scatter(x_points, y_points, color='blue', zorder=3, label='the calculated points')
